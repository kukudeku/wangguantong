<template>
  <a-card class="page-card" title="上机管理">
    ...
    <a-table class="no-wrap-table" :columns="columns" :data="records" row-key="id" :pagination="false" :scroll="{ x: 1510 }">
...
async function loadOptions() {
  ...
  const computers = await getComputerList({})
  freeComputers.value = computers.filter((item) => item.status === '空闲')
}

...
-- 为旧会员表补充用户类型和会员等级。注意：此段不是幂等语句，已有字段时不要重复执行。
ALTER TABLE `member`
...
-- 查询 information_schema 判断旧 food_item 表是否已有 category 字段。
...
-- 根据查询结果动态选择 ALTER TABLE 或无操作的 SELECT 1，实现可重复执行。
...
-- PREPARE/EXECUTE 执行上面拼出的动态 SQL，随后释放临时语句。
...
-- SELECT ... WHERE NOT EXISTS 只在商品不存在时插入，避免重复测试数据。

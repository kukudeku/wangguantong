...
// false 显示登录，true 显示注册；点击顶部切换按钮会修改该值。
const isRegister = ref(false)
// 测试环境预填演示账号，正式使用时可删除默认值。
const form = reactive({
  username: '110101199001011234',
  password: '123456'
})
// 注册提交姓名、身份证号、手机号和密码；身份证号也是以后登录的账号。
...
async function handleLogin() {
  // 浏览器先检查必填，再交给后端核对身份证号和密码。
  if (!form.username || !form.password) {
    Message.error('请输入身份证号和密码')
    return
  }
  const user = await userLogin(form)
  // 保存完整用户对象，路由守卫和用户端首页都从 localStorage 读取登录状态。
